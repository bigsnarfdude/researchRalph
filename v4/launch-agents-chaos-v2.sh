...
REPO_ROOT="/home/vincent/researchRalphLocal"
...
for i in $(seq 0 $((NUM_AGENTS - 1))); do
    ...
    if [[ "$CHAOS_LOOKUP" == *",$i,"* ]]; then
        ...
        screen -dmS "$SESSION" bash -c "python3 $REPO_ROOT/rrma_local/tools/honest_agent_llama.py $DOMAIN_DIR --agent-id $i --turns $MAX_TURNS > $DOMAIN_DIR/logs/agent${i}.log 2>&1"
    fi
    sleep 15
done
