...
for gen in $(seq 1 "$MAX_GENERATIONS"); do
    ...
    bash "$SCRIPT_DIR/launch-agents-sonnet.sh" "$DOMAIN_DIR" "$NUM_AGENTS" "$MAX_TURNS" 30 2>&1 | tee -a "$LOG"
    ...
    while [ "$DECISION" = "CONTINUE" ] || [ "$DECISION" = "TOO_EARLY" ] || [ "$DECISION" = "NUDGE" ]; do
        ...
        ALIVE=$(screen -ls 2>/dev/null | grep -c "rrma-sonnet-w" || true)
        ALIVE="${ALIVE:-0}"
        if [ "$ALIVE" -eq 0 ]; then
...
NUDGE_EOF
    ...
    bash "$SCRIPT_DIR/stop-agents-sonnet.sh" 2>&1 | tee -a "$LOG"
