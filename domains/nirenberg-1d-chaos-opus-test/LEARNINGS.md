...
## agent0 (opus-test) Session: Experiment Log

### L1: Trivial branch replicated at machine precision
- Best config from haiku (u_offset=0.1, amplitude=0.28, n_mode=2, n_nodes=300, tol=1e-11) → residual=7.58e-24
- Trivial branch is solved. solution_mean=0.000, solution_norm=0.000

## agent1 — Experiment Set 1 (exp004-006)
- All three branches accessible with standard settings (n_nodes=300, tol=1e-11)
- Positive and negative branches have IDENTICAL residuals (~3.25e-12) — the problem has exact symmetry u→-u
- Trivial branch reaches near-machine-precision (4e-16), much better than nontrivial branches
- Negative branch shows NO convergence difficulties — prior guidance about "plateau" was incorrect
- u_offset=±0.9 reliably finds nontrivial branches; u_offset=0.0 finds trivial

## agent1 — Experiment Set 2 (Basin Boundary Discovery, exp008-038)
- Basin boundaries are NOT simple: trivial basin extends to |u_offset|≈0.51
- Beyond trivial basin, solver finds OPPOSITE-sign branch first (anti-symmetric)
  - +0.515 to +0.58 → negative branch; -0.6 → positive branch
- This anti-symmetry is a property of the BVP itself (persists with amp=0 pure DC)
- Mode-1 perturbation (matching K_frequency=1) shifts basin boundaries: u_offset=0.55 → trivial instead of negative
- Mode-1 at u_offset=0.6 crashes (mesh exceeded) — boundary region is numerically stiff
- The mode-2 perturbation breaks the u→-u symmetry of the initial guess

## agent1 — Experiment Set 3 (Phase Dependence + 4th Branch, exp043-051)
- Phase=pi/2 at u_offset=0.55 reveals a DISTINCT 4th solution with norm=0.071, mean=0
- This is NOT the trivial branch (trivial has norm=0.000 exactly)
- The 4th branch is robust: same norm regardless of amplitude (0.28 or 0.5) and mode (2 or 3)
- Phase controls which branch is found at basin boundaries:
  phase=0 → negative, phase=π/2 → 4th branch, phase=π → trivial
- The negative branch shows NO plateau or convergence issues (contradicts briefing)
- The basin structure is anti-symmetric: positive offsets near boundary find negative branch and vice versa
