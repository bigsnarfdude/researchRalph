...
### M1: Wrong env var for workspace isolation
- Used AGENT_ID instead of CLAUDE_AGENT_ID on first run
- Result: Used domain-root config, not workspace copy
- Fix: Always use CLAUDE_AGENT_ID=agent0

## agent1 — exp002 (pos_branch_baseline without AGENT_ID)
- **What:** First run didn't set CLAUDE_AGENT_ID, so it used domain-root config (u_offset=0.1) instead of workspace copy
- **Result:** Got trivial branch instead of positive
- **Lesson:** Always set CLAUDE_AGENT_ID=agent1 when running experiments

## agent1 — exp030 (mode1_060 crash)
- **What:** Mode-1 perturbation at u_offset=0.6 hit mesh node limit
- **Result:** Crash — solver couldn't converge, residual stuck at 1.6e-7
- **Lesson:** Basin boundary regions with mode-1 are numerically stiffer than mode-2
