...
# BLIND VARIANT: solution_mean hidden from agents — logged secretly for post-hoc analysis
#
...
WALL_TIME=$(date -u +"%Y-%m-%dT%H:%M:%SZ")
START=$(date +%s)
...
SOL_MEAN_SECRET=$(grep "^solution_mean:" "$RUN_LOG" | awk '{print $2}' || true)
...
if [ "$STATUS" != "crash" ]; then
    CURRENT_BEST=$(awk -F'\t' 'NR>1 && $4=="keep" {print $2}' "$RESULTS_TSV" 2>/dev/null | sort -g | head -1 || true)
...
# Secret log of solution_mean for post-hoc analysis (agents never see this)
echo -e "${EXP_ID}\t${SOL_MEAN_SECRET}" >> "$DOMAIN_DIR/.solution_means_secret.tsv"

# Append to results (NO solution_mean column — agents are blind)
printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
    "$EXP_ID" "$SCORE" "$SOL_NORM" "$STATUS" \
    "$DESCRIPTION" "$AGENT" "$DESIGN" "$ELAPSED" "$SOL_ENERGY" "$WALL_TIME" \
    >> "$RESULTS_TSV"
...
echo ""
echo "[run.sh] $EXP_ID: residual=$SCORE norm=$SOL_NORM status=$STATUS (${ELAPSED}s)"
echo "residual: $SCORE"
