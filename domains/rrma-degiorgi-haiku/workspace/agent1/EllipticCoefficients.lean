...
theorem det_ne_zero_of_coercive {x : AmbientSpace d}
    ...
    (A.a x).det ≠ 0 := by
  intro hdim
  -- If det = 0, there exists nonzero ξ with Aξ = 0
  by_contra h
  push_neg at h
  -- Use coercivity to derive contradiction
  sorry
...
theorem inv_matMulE_matMulE {x : AmbientSpace d}
  ...
  -- Prove det ≠ 0 from coercivity
  have h_det : (A.a x).det ≠ 0 := A.det_ne_zero_of_coercive hx
  -- matMulE is composable: matMulE A⁻¹ (matMulE A ξ) = matMulE (A⁻¹ * A) ξ = matMulE 1 ξ = ξ
  -- Use functional extensionality for EuclideanSpace (which is Fin d → ℝ)
  ext i
  -- Apply component-wise
  simp only [matMulE, EuclideanSpace.toLp_apply]
  -- Now we need to show: WithLp.toLp 2 (mulVec A⁻¹ (mulVec A ξ.ofLp)) i = WithLp.toLp 2 ξ.ofLp i
  rw [Matrix.mulVec_mulVec, Matrix.inv_mul_cancel _ h_det, Matrix.mul_one, Matrix.mulVec_one]
