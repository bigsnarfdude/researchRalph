...
# Untried method family: every prior class in this 135+ experiment campaign is
# BatchTopK-lineage (global scalar cutoff). JumpReLU uses a per-feature
# learned threshold instead — a different sparsity mechanism entirely. Paired
# here with Matryoshka nested-width reconstruction losses (arxiv 2503.17547).
#
# EXP-001 attempted this and crashed at save time: sae_lens's
# JumpReLUTrainingSAEConfig.get_inference_config_class has no BatchTopK-style
# hardcode, so it falls through to a registry lookup on architecture() —
# and a custom architecture string there is a guaranteed KeyError (see
# MISTAKES.md for the full sae_lens source trace). Fix: do NOT override
# architecture(); leave it at the inherited "jumprelu" (a name sae_lens's
# registry actually knows), so save_inference_model resolves cleanly.
...
@dataclass
class JumpReLUMatryoshkaConfig(JumpReLUTrainingSAEConfig):
    """JumpReLU activation + Matryoshka nested reconstruction losses."""

    matryoshka_widths: list[int] = field(default_factory=list)

    @classmethod
    def from_dict(cls, cfg: dict, total_steps: int = 0) -> "JumpReLUMatryoshkaConfig":
        d_sae = int(cfg.get("d_sae", 4096))
        default_widths = [256, 512, 1024, 2048, d_sae]
        widths = list(cfg.get("matryoshka_widths", default_widths))
        pre_act_coef = cfg.get("pre_act_loss_coefficient", 3e-6)
        return cls(
            d_in=D_IN,
            d_sae=d_sae,
            matryoshka_widths=widths,
            ...
            jumprelu_tanh_scale=float(cfg.get("jumprelu_tanh_scale", 4.0)),
            l0_coefficient=float(cfg.get("l0_coefficient", 3e-4)),
            l0_warm_up_steps=int(cfg.get("l0_warm_up_steps", 1000)),
            pre_act_loss_coefficient=(
                float(pre_act_coef) if pre_act_coef is not None else None
            ),
            dtype="float32",
            device="cuda",
        )
...
class JumpReLUMatryoshka(JumpReLUTrainingSAE):
    """JumpReLU + Matryoshka: nested reconstruction losses on top of a
    per-feature learned-threshold activation, instead of BatchTopK."""

    cfg: JumpReLUMatryoshkaConfig

    def __init__(self, cfg: JumpReLUMatryoshkaConfig, use_error_term: bool = False):
        super().__init__(cfg, use_error_term)
        if cfg.matryoshka_widths[-1] != cfg.d_sae:
            cfg.matryoshka_widths.append(cfg.d_sae)

    def _iterable_decode(self, feature_acts: torch.Tensor, include_outer_loss: bool = False):
