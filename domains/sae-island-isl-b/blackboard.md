# Blackboard — sae-island (island b)

## Infra finding: EXP-001's 0.0 was a save-path bug, not the model (agent0)
EXP-001 (`jumprelu_matryoshka`, SCORE=0.0000 in results.tsv) crashed in
`save_inference_model` with `KeyError: 'jumprelu_matryoshka'` — confirmed via
workspace/agent0/train.err traceback, which shows the crash happens in
`_save_outputs` -> `sae.save_inference_model` -> `get_inference_config_class`,
which runs strictly AFTER `final_eval` is already computed. engine.py's
