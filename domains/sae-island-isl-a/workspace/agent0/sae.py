...
# --- JumpReLU SAE ---
# Per-feature learned threshold via an L0 penalty (not a fixed global-k cutoff),
# unlike every other class in this file (all BatchTopK/Matryoshka/LISTA/MatchingPursuit
# descendants). May recover rare GT features better than a shared per-token budget.
...
@dataclass
class JumpReLUSAEConfig(JumpReLUTrainingSAEConfig):
    @classmethod
    def from_dict(cls, cfg: dict, total_steps: int) -> "JumpReLUSAEConfig":
        return cls(
            d_in=D_IN,
            d_sae=int(cfg['d_sae']),
            dtype="float32",
            ...
            jumprelu_sparsity_loss_mode=cfg.get('jumprelu_sparsity_loss_mode', 'step'),
            l0_coefficient=float(cfg.get('l0_coefficient', 1.0)),
            l0_warm_up_steps=int(cfg.get('l0_warm_up_steps', 0)),
            pre_act_loss_coefficient=cfg.get('pre_act_loss_coefficient', None),
        )


class JumpReLUSAE(JumpReLUTrainingSAE):
    """JumpReLU SAE wrapper for engine.py compatibility."""
...
# --- Gated SAE ---
# Separate gating (binary, which features fire) and magnitude (how much) paths,
# tied via W_enc with an independent r_mag scale. Sparsity comes from an L1
# penalty on the ReLU'd gating pre-activation (scaled by decoder norms), plus
# an auxiliary reconstruction-via-gate-path loss. Architecturally distinct
# from both hard-TopK (fixed k) and JumpReLU (learned per-feature threshold,
# ceiling ~0.70 per EXP-001..010) — untried method family per LEARNINGS.md.
...
@dataclass
class GatedSAEConfig(GatedTrainingSAEConfig):
    @classmethod
    def from_dict(cls, cfg: dict, total_steps: int) -> "GatedSAEConfig":
        return cls(
            d_in=D_IN,
            d_sae=int(cfg['d_sae']),
            dtype="float32",
...
class GatedSAE(GatedTrainingSAE):
    """Gated SAE wrapper for engine.py compatibility."""
    cfg: GatedSAEConfig
