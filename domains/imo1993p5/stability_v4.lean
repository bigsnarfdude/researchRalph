  ...
  | h n ih =>
    if h0 : n = 0 then
      ...
      have hk_diff : (f (k + 1) : ℤ) - (f k : ℤ) = 1 ∨ (f (k + 1) : ℤ) - (f k : ℤ) = 2 := by
        rcases h_jump_k with h1 | h2 <;> omega
...
theorem stability (f g : ℕ → ℕ) (hf : IsSolution f) (hg : IsSolution g) (n : ℕ) :
  ...
  | h n ih =>
    if hn0 : n = 0 then subst hn0; rw [f0_eq_0 f hf, f0_eq_0 g hg]; simp
      ...
      have h_fn_v : b f (n - 1) = (f n : ℤ) - (n : ℤ) - 1 := by
        unfold b; rw [h_n1]
      have h_gn_v : b g (n - 1) = (g n : ℤ) - (n : ℤ) - 1 := by
        unfold b; rw [h_n1]

      rw [abs_le]; constructor
      · -- f n - g n <= 1
        by_contra h_contr; have h_diff : (f n : ℤ) - (g n : ℤ) ≥ 2 := by omega
        have h_b_diff : b f (n - 1) - b g (n - 1) ≥ 2 := by rw [h_fn_v, h_gn_v]; omega
      ...
      · -- g n - f n <= 1
        by_contra h_contr; have h_diff : (g n : ℤ) - (f n : ℤ) ≥ 2 := by omega
        have h_b_diff : b g (n - 1) - b f (n - 1) ≥ 2 := by rw [h_fn_v, h_gn_v]; omega
