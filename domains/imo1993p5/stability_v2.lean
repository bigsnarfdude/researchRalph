...
theorem jump_morphism (f : ℕ → ℕ) (h : IsSolution f) (n : ℕ) :
  ...
  have h_f_f : (f (f (n + 1)) : ℤ) = (f (n + 1) : ℤ) + (n : ℤ) := by
    rw [← h.2.1 (n+1)]; simp; rw [h.2.1]
  have h_f_f_n : (f (f n) : ℤ) = (f n : ℤ) + (n : ℤ) := by
    rw [← h.2.1 n]; simp; rw [h.2.1]
  ...
  | h n ih =>
    if h0 : n = 0 then
      ...
      have hk_next : n < f (k + 1) := by
        ...
        have : k + 1 ∈ S := by
           simp; constructor
           · have : f (k+1) > f k := h.2.2 k
             have : f k ≤ n := hk
             have : n < f n := f_gt_n f h n (by omega)
      ...
      have hk_diff : (f (k + 1) : ℤ) - (f k : ℤ) = 1 ∨ (f (k + 1) : ℤ) - (f k : ℤ) = 2 := by
        ...
        · left; rw [Nat.cast_sub (h.2.2 k |>.le)] at *; exact Nat.cast_inj.mpr h1
        · right; rw [Nat.cast_sub (h.2.2 k |>.le)] at *; exact Nat.cast_inj.mpr h2
      ...
      · -- f(k+1)-f(k) = 2. Then n = f(k) or n = f(k)+1.
        ...
        · -- n = f k
          rw [Nat.cast_sub (h.2.2 n |>.le)]
          exact Nat.cast_inj.mp (by omega)
        · -- n = f k + 1
          rw [Nat.cast_sub (h.2.2 n |>.le)]
          exact Nat.cast_inj.mp (by omega)
    ...
    if hn0 : n = 0 then subst hn0; rw [f0_eq_0 f hf, f0_eq_0 g hg]; simp
      ...
      have h_ij : |(i : ℤ) - (j : ℤ)| ≤ 1 := by
        ...
        · by_contra h_contr; have : (i : ℤ) - (j : ℤ) ≥ 2 := by omega
          ...
          have h_ih := ih (j + 2) (by
            if hj2_lt3 : j + 2 < 3 then omega
            else have := h_g_ge_m2 (j + 2) (by omega); omega)
        ...
        · by_contra h_contr; have : (j : ℤ) - (i : ℤ) ≥ 2 := by omega
          ...
          have h_ih := ih (i + 2) (by
            if hi2_lt3 : i + 2 < 3 then omega
            else have := h_f_ge_m2 (i + 2) (by omega); omega)
          have : f (i + 1) < f (i + 2) := hf.2.2 (i + 1)
          rw [abs_le] at h_ih; omega

      have h_fn : (f n : ℤ) = (n : ℤ) + 1 + b f (n - 1) := by 
        unfold b; have h_eq : n - 1 + 1 = n := by omega
        have h_cast : (n : ℤ) = (n - 1 : ℤ) + 1 := by omega
        rw [h_eq]; ring
      ...
      · -- f n - g n <= 1
        ...
        have h_ih_j1 := ih (j + 1) (by
           if hj1_lt3 : j + 1 < 3 then omega
           else have := h_g_ge_m2 (j + 1) (by omega); omega)
      ...
      · -- g n - f n <= 1
        ...
        have h_ih_i1 := ih (i + 1) (by
           if hi1_lt3 : i + 1 < 3 then omega
           else have := h_f_ge_m2 (i + 1) (by omega); omega)
