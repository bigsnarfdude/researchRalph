...
-- Local replacements for FormalConjectures.Util.ProblemImports
-- IsSyndetic: bounded gaps — ∃ window p, every interval [x, x+p] hits S
def IsSyndetic (S : Set ℕ) : Prop :=
  ∃ p : ℕ, ∀ x : ℕ, ∃ y, y ∈ S ∧ y ∈ Icc x (x + p)

-- IsAddBasisOfOrder: h-fold sumset covers all of ℕ
-- Must be definitionally equal to (∀ n, n ∈ h • S) for basis_of_order_two_iff by rfl
def IsAddBasisOfOrder (S : Set ℕ) (h : ℕ) : Prop :=
  ∀ n : ℕ, n ∈ h • S

-- HasLargeGaps: ∀ C, ∃ window of length C with no element of S
...
lemma seq_step_prop (n : ℕ) : step_prop (seq_step n) n (seq_step (n + 1)) :=
...
lemma greedy_seq_exists : ∃ (f : ℕ → Set ℕ) (gap_end : ℕ → ℕ),
  IsGreedyBasis f ∧ GreedySpaced f gap_end ∧ GreedyGaps f gap_end :=
  ⟨f_seq, gap_seq, f_seq_basis, f_seq_spaced, f_seq_gaps⟩
...
lemma subset_f_k_of_le (f : ℕ → Set ℕ) (gap_end : ℕ → ℕ) (h_spaced : GreedySpaced f gap_end) (k : ℕ) :
  ...
  · exfalso
    ...
    have h_gap : y > gap_end (m - 1) := by
      ...
      have h_diff' : y ∈ f ((m - 1) + 1) \ f (m - 1) := by rw [← h_m_eq]; exact h_diff
      exact h_spaced.2.2 (m - 1) y h_diff'
    have h_gap_mono : gap_end k ≤ gap_end (m - 1) := gap_mono f gap_end h_spaced (by omega)
    linarith

lemma erdos_gap_set_exists : ∃ A : Set ℕ, IsAddBasisOfOrder (A ∪ {0}) 2 ∧
    ∀ A₁ A₂, A = A₁ ∪ A₂ → Disjoint A₁ A₂ → HasLargeGaps (A₁ + A₁) ∨ HasLargeGaps (A₂ + A₂) := by
  ...
  · intros A₁ A₂ h_part h_disj
    have h_or_C : ∀ C : ℕ, (∃ N, ∀ x, N ≤ x → x ≤ N + C → x ∉ A₁ + A₁) ∨
      ...
      | inl h_inl =>
        ...
        have ha_gap : a ≤ gap_end k := le_trans (by linarith) hN_le
        have hb_gap : b ≤ gap_end k := le_trans (by linarith) hN_le
    ...
    cases infinite_or P Q h_monoP h_monoQ h_or_C with
...
theorem erdos_741_ii :
    ∃ A : Set ℕ, IsAddBasisOfOrder (A ∪ {0}) 2 ∧
    ∀ A₁ A₂, A = A₁ ∪ A₂ → Disjoint A₁ A₂ →
  ...
  cases h_cases with
  | inl h1 =>
    exact not_syndetic_of_large_gaps (A₁ + A₁) h1 h_syn1
  | inr h2 =>
    exact not_syndetic_of_large_gaps (A₂ + A₂) h2 h_syn2
