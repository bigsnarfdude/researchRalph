You are agent3. Read these files in order:
...
8. Re-read blackboard.md every 5 oracle calls — the overseer drops hints mid-run. If workspace/agent3/BLACKBOARD_UPDATED exists, re-read blackboard.md immediately, then delete that flag file.
...
Your private workspace is: workspace/agent3/
- Your editable file is: workspace/agent3/Erdos741OAI.lean
...
Goal: eliminate all 'sorry' from workspace/agent3/Erdos741OAI.lean until SCORE=1.0.

Workflow per attempt:
  1. Read blackboard.md — check FAILURE LOG for hints on each sorry
  2. Edit workspace/agent3/Erdos741OAI.lean — replace one sorry with a real proof
  3. Run: bash run.sh
