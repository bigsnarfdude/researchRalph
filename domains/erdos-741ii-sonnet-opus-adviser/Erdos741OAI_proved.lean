...
lemma akn_subset_setA (k : ℕ) : Akn k ⊆ setA := by
  ...
  | succ k ih =>
    ...
    · exact ih h
    · exact Or.inr (Set.mem_iUnion.mpr ⟨k, Or.inl (Or.inl h)⟩)
...
lemma basis_lem (k : ℕ) : Icc 4 (6 * Q k) ⊆ Akn (k + 1) + Akn (k + 1) := by
  ...
  | zero =>
    intro x hx
    simp only [Q, pow_zero, Nat.mul_one, mem_Icc] at hx
    -- x ∈ [4,6], show via 2+2, 2+3, 3+3
    have h2 : (2 : ℕ) ∈ Akn 1 := by
      apply @akn_mono 0 1 (by omega)
      show (2 : ℕ) ∈ ({2, 3} : Set ℕ)
      exact mem_insert 2 {3}
    have h3 : (3 : ℕ) ∈ Akn 1 := by
      apply @akn_mono 0 1 (by omega)
      show (3 : ℕ) ∈ ({2, 3} : Set ℕ)
      exact mem_insert_of_mem 2 rfl
    simp only [Set.mem_add]
    obtain ⟨hxlo, hxhi⟩ := hx
    interval_cases x
    · exact ⟨2, h2, 2, h2, rfl⟩
...
lemma rigidity_lem (k : ℕ) (n : ℕ) (hn : n ∈ Jk k)
  ...
  · -- a ∈ {2,3}, b ∈ stage i
    ...
    · -- i = k: b ∈ stage k
      ...
      · simp only [Bk, mem_Icc] at hbi; omega
  ...
  · -- a ∈ stage j, b ∈ {2,3}: symmetric to case 2
    ...
    · rw [hje] at haj
      ...
      · simp only [Bk, mem_Icc] at haj; omega
  ...
  · -- a ∈ stage j, b ∈ stage i
    ...
    · -- j < k: a ≤ 3Qk, b ≥ 6Qk
      ...
      · -- i = k
        ...
        rcases hbi with ((hbi | hbi) | hbi)
        · simp only [mem_singleton_iff] at hbi
          -- b = ck k = 4Qk, but hbdown_val : 6*Qk ≤ b. Contradiction.
          have hval : b = 4 * Q k := hbi ▸ rfl
          linarith
        · simp only [Bk, mem_Icc] at hbi
          -- b ≤ 6Qk-1 < 6Qk ≤ b. Contradiction via nat sub.
          omega
        · simp only [Fk, mem_Icc] at hbi
          have hbnn : b ≤ n := by omega
          have halow : 4 * Q j ≤ a := stage_lower haj
          have hQj : 0 < Q j := Q_pos j
          have hblo : 10 * Q k - 1 ≤ b := hbi.1
          omega
    ...
    · -- j = k: a ∈ stage k
      ...
      · -- a ∈ Bk k = [5Qk, 6Qk-1]
        ...
        · rw [hije] at hbi
          ...
          · -- b ∈ Fk k: a+b ≥ 5Qk+(10Qk-1) > 10Qk > n. Contradiction.
            simp only [Fk, mem_Icc] at hbi
            omega
