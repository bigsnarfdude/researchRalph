...
if [ -f "$WORKSPACE_FILE" ]; then
    ...
    SOLUTION="$DOMAIN_DIR/Erdos741OAI.lean"
fi
...
THM_PRESENT=$(grep -c "theorem erdos_741_ii" "$SOLUTION")
GOAL_PRESENT=$(grep -c "A₁ ∩ A₂ = ∅" "$SOLUTION")
...
if [ "$SORRY_COUNT" -eq 0 ] && [ "$BUILD_EXIT" -eq 0 ] && [ "$THM_PRESENT" -ge 1 ] && [ "$GOAL_PRESENT" -ge 1 ]; then
