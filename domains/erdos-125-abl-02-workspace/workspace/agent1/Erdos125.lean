...
-- PHASE 2: Generalization to bases (10, 12)
def setA1012 : Set ℕ := {n | ∀ d ∈ Nat.digits 10 n, d ≤ 1}
def setB1012 : Set ℕ := {n | ∀ d ∈ Nat.digits 12 n, d ≤ 1}
def setAB1012 : Set ℕ := {n | ∃ a ∈ setA1012, ∃ b ∈ setB1012, a + b = n}

private lemma setA1012_le_181 {n : ℕ} (hn : n ∈ setA1012) (hlt : n < 1000) : n ≤ 181 := by
  simp only [setA1012, Set.mem_setOf_eq] at hn
  have key : ∀ m ∈ Finset.range 1000, (∀ d ∈ Nat.digits 10 m, d ≤ 1) → m ≤ 181 := by
    native_decide
  exact key n (Finset.mem_range.mpr hlt) hn

private lemma setB1012_le_157 {n : ℕ} (hn : n ∈ setB1012) (hlt : n < 1728) : n ≤ 157 := by
  simp only [setB1012, Set.mem_setOf_eq] at hn
  have key : ∀ m ∈ Finset.range 1728, (∀ d ∈ Nat.digits 12 m, d ≤ 1) → m ≤ 157 := by
    native_decide
  exact key n (Finset.mem_range.mpr hlt) hn

lemma gap_exists_1012 : ∃ n : ℕ, n ∉ setAB1012 := by
  use 339
  simp only [setAB1012, Set.mem_setOf_eq]
  rintro ⟨a, ha_A, b, hb_B, hab⟩
  have ha_bound : a ≤ 181 := setA1012_le_181 ha_A (by omega)
  have hb_bound : b ≤ 157 := setB1012_le_157 hb_B (by omega)
  omega
