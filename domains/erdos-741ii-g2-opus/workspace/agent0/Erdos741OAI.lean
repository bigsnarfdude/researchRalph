...
-- Given two intervals whose sum-range contains x, exhibit a witness pair.
lemma pair_mem {S T : Set ℕ} {lo1 hi1 lo2 hi2 x : ℕ}
    (hS : Icc lo1 hi1 ⊆ S) (hT : Icc lo2 hi2 ⊆ T)
    (h1 : lo1 ≤ hi1) (h2 : lo2 ≤ hi2)
    (hx : lo1 + lo2 ≤ x ∧ x ≤ hi1 + hi2) : x ∈ S + T := by
  obtain ⟨hlo, hhi⟩ := hx
  simp only [Set.mem_add]
  by_cases h : x ≤ lo1 + hi2
  · exact ⟨lo1, hS (mem_Icc.mpr ⟨le_refl _, h1⟩), x - lo1,
      hT (mem_Icc.mpr ⟨by omega, by omega⟩), by omega⟩
  · exact ⟨x - hi2, hS (mem_Icc.mpr ⟨by omega, by omega⟩), hi2,
      hT (mem_Icc.mpr ⟨h2, le_refl _⟩), by omega⟩

-- Covers [4Qk, 30Qk] using the eight pair types from {I, ck, Bk, Fk} at level k.
-- I = [2Qk, 3Qk], ck = 4Qk, Bk = [5Qk, 6Qk-1], Fk = [10Qk-1, 15Qk]
-- I+I=[4Q,6Q], I+ck=[6Q,7Q], I+Bk=[7Q,9Q-1], ck+Bk=[9Q,10Q-1],
-- Bk+Bk=[10Q,12Q-2], I+Fk=[12Q-1,18Q], Bk+Fk=[15Q-1,21Q-1], Fk+Fk=[20Q-2,30Q]
lemma cover_high (k : ℕ) : Icc (4 * Q k) (30 * Q k) ⊆ Akn (k + 1) + Akn (k + 1) := by
  have hQp : 0 < Q k := Q_pos k
  have hI : Icc (2 * Q k) (3 * Q k) ⊆ Akn (k + 1) := ik_sub_akn k
  have hck : Icc (4 * Q k) (4 * Q k) ⊆ Akn (k + 1) := by
    intro y hy
    simp only [mem_Icc] at hy
    have hy' : y = 4 * Q k := by omega
    simp only [Akn]
    rw [hy']
    exact Or.inl (Or.inl (Or.inr (Set.mem_singleton_iff.mpr rfl)))
  have hBk : Icc (5 * Q k) (6 * Q k - 1) ⊆ Akn (k + 1) := by
    intro y hy; simp only [Akn]; exact Or.inl (Or.inr hy)
  have hFk : Icc (10 * Q k - 1) (15 * Q k) ⊆ Akn (k + 1) := by
    intro y hy; simp only [Akn]; exact Or.inr hy
  intro x hx
  simp only [mem_Icc] at hx
  obtain ⟨hlo, hhi⟩ := hx
  by_cases h6 : x ≤ 6 * Q k
  · exact pair_mem hI hI (by omega) (by omega) ⟨by omega, by omega⟩
  by_cases h7 : x ≤ 7 * Q k
  · exact pair_mem hI hck (by omega) (by omega) ⟨by omega, by omega⟩
  by_cases h9 : x ≤ 9 * Q k - 1
  · exact pair_mem hI hBk (by omega) (by omega) ⟨by omega, by omega⟩
  by_cases h10 : x ≤ 10 * Q k - 1
  · exact pair_mem hck hBk (by omega) (by omega) ⟨by omega, by omega⟩
  by_cases h12 : x ≤ 12 * Q k - 2
  · exact pair_mem hBk hBk (by omega) (by omega) ⟨by omega, by omega⟩
  by_cases h18 : x ≤ 18 * Q k
  · exact pair_mem hI hFk (by omega) (by omega) ⟨by omega, by omega⟩
  by_cases h21 : x ≤ 21 * Q k - 1
  · exact pair_mem hBk hFk (by omega) (by omega) ⟨by omega, by omega⟩
  · exact pair_mem hFk hFk (by omega) (by omega) ⟨by omega, by omega⟩
...
lemma basis_lem (k : ℕ) : Icc 4 (6 * Q k) ⊆ Akn (k + 1) + Akn (k + 1) := by
  ...
  | zero =>
    ...
    obtain ⟨hx1, hx2⟩ := hx
    have hI23 : Icc 2 3 ⊆ Akn (0 + 1) := by
      intro y hy; simp only [Akn]; exact Or.inl (Or.inl (Or.inl hy))
    exact pair_mem hI23 hI23 (by norm_num) (by norm_num) ⟨by omega, by omega⟩
  | succ k ih =>
    intro x hx
    simp only [mem_Icc] at hx
    obtain ⟨hx1, hx2⟩ := hx
    ...
    · exact Set.add_subset_add (akn_mono (Nat.le_succ _)) (akn_mono (Nat.le_succ _))
        (ih (mem_Icc.mpr ⟨hx1, hlow⟩))
    · have hch : x ∈ Akn (k + 1) + Akn (k + 1) := by
        apply cover_high k
        simp only [mem_Icc]
        rw [hQs] at hx2
        omega
      exact Set.add_subset_add (akn_mono (Nat.le_succ _)) (akn_mono (Nat.le_succ _)) hch
