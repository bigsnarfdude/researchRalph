...
/-!
## PHASE 2: Instantiation for (3,5) base pair

Testing whether the proof generalizes to multiplicatively independent base pairs.
Here: A_3 = {n with only 0,1 digits in base 3}, B_5 = {n with only 0,1 digits in base 5}
Result: lowerDensity(A_3 + B_5) = 0

(We use (3,5) rather than (2,3) because base 2 only has digits {0,1}, making the condition vacuous.)
-/

namespace Phase2_3_5

-- A_3: natural numbers with only digits 0,1 in base 3
def setA3 : Set ℕ := {n | ∀ d ∈ Nat.digits 3 n, d ≤ 1}

-- B_5: natural numbers with only digits 0,1 in base 5
def setB5 : Set ℕ := {n | ∀ d ∈ Nat.digits 5 n, d ≤ 1}

-- Sumset A_3 + B_5
def setA3B5 : Set ℕ := {n | ∃ a ∈ setA3, ∃ b ∈ setB5, a + b = n}

/-!
## Sub-lemma: 3 and 5 are multiplicatively independent
-/
private lemma nat_pow_ne_3_5 (b a : ℕ) (hb : 0 < b) (ha : 0 < a) :
    (3 : ℕ) ^ b ≠ (5 : ℕ) ^ a := by
  intro h_eq
  have hcop : Nat.Coprime 3 (5 ^ a) := (by decide : Nat.Coprime 3 5).pow_right _
  have h3_dvd_5a : (3 : ℕ) ∣ 5 ^ a := h_eq ▸ dvd_pow_self 3 hb.ne'
  have h3_dvd_1 : (3 : ℕ) ∣ 1 := hcop ▸ Nat.dvd_gcd (dvd_refl 3) h3_dvd_5a
  exact absurd h3_dvd_1 (by decide)

/-!
## Dirichlet approximation for (3,5)
-/
lemma exists_k_m_ratio_close_3_5 (ε : ℝ) (hε : 0 < ε) :
    ∃ k m : ℕ, 0 < k ∧ 0 < m ∧ |↑k * log 3 - ↑m * log 5| < ε := by
  have hlog3_pos : (0 : ℝ) < log 3 := Real.log_pos (by norm_num)
  have hlog5_pos : (0 : ℝ) < log 5 := Real.log_pos (by norm_num)
  -- log 3 / log 5 is irrational
  have hirr : Irrational (log 3 / log 5) := by
    rw [irrational_iff_ne_rational]
    intro a b hb heq
    have hb_real : (b : ℝ) ≠ 0 := Int.cast_ne_zero.mpr hb
    have h_mul : (b : ℝ) * log 3 = (a : ℝ) * log 5 := by
    ...
    have hb' : 0 < b.natAbs := Int.natAbs_pos.mpr hb
    have ha' : 0 < a.natAbs := Int.natAbs_pos.mpr ha_ne
    have h_natabs : (b.natAbs : ℝ) * log 3 = (a.natAbs : ℝ) * log 5 := by
      rw [Nat.cast_natAbs, Nat.cast_natAbs, Int.cast_abs, Int.cast_abs]
      obtain hb_nn | hb_neg := le_or_gt 0 (b : ℝ)
      · rw [abs_of_nonneg hb_nn]
        have ha_nn : 0 ≤ (a : ℝ) := by nlinarith
      ...
      · rw [abs_of_neg hb_neg]
        have ha_neg : (a : ℝ) < 0 := by nlinarith
        rw [abs_of_neg ha_neg]; linarith
    have h_rpow : (3 : ℝ) ^ b.natAbs = (5 : ℝ) ^ a.natAbs := by
      apply Real.log_injOn_pos (Set.mem_Ioi.mpr (by positivity))
                               (Set.mem_Ioi.mpr (by positivity))
      rw [Real.log_pow, Real.log_pow]
      exact_mod_cast h_natabs
    have h_nat : (3 : ℕ) ^ b.natAbs = (5 : ℕ) ^ a.natAbs := by exact_mod_cast h_rpow
    have hcop : Nat.Coprime 3 (5 ^ a.natAbs) := (by decide : Nat.Coprime 3 5).pow_right _
    have h3_dvd : (3 : ℕ) ∣ 5 ^ a.natAbs := h_nat ▸ dvd_pow_self 3 hb'.ne'
    exact absurd (hcop ▸ Nat.dvd_gcd (dvd_refl 3) h3_dvd) (by decide)
  -- Dirichlet approximation
  obtain ⟨N, hN⟩ := exists_nat_gt (log 5 / ε)
  have hN_pos : 0 < N + 1 := Nat.succ_pos _
  obtain ⟨j, k, hk_pos, _, hbound⟩ :=
    Real.exists_int_int_abs_mul_sub_le (log 3 / log 5) hN_pos
  -- 1/(N+2) < ε/log5
  have hN2_bound : (1 : ℝ) / (↑(N + 1) + 1) < ε / log 5 := by
    ...
    push_cast; linarith
  -- j > 0: log 3 / log 5 ≈ 0.683, so (1/2, 1)
  have hj_pos : 0 < j := by
    have hk_real : (1 : ℝ) ≤ (k : ℝ) := by exact_mod_cast hk_pos
    have hξ_pos : 0 < log 3 / log 5 := div_pos hlog3_pos hlog5_pos
    have hξ_gt_half : (1:ℝ)/2 < log 3 / log 5 := by
      rw [lt_div_iff₀ hlog5_pos]
      have h1 : log 5 < log 9 := Real.log_lt_log (by norm_num) (by norm_num)
      have h2 : log (9:ℝ) = 2 * log 3 := by
        have : (9:ℝ) = 3 ^ 2 := by norm_num
        rw [this, Real.log_pow]; norm_cast
      linarith
    have hkξ_gt_half : (1:ℝ)/2 < (k:ℝ) * (log 3 / log 5) := by
      nlinarith [mul_nonneg (show (0:ℝ) ≤ (k:ℝ) - 1 by linarith) (le_of_lt hξ_pos)]
    have h_half : (1:ℝ) / (↑(N+1) + 1) ≤ 1/2 := by
      have hd : (0:ℝ) < ↑(N+1) + 1 := by positivity
      have h2le : (2:ℝ) ≤ ↑(N+1) + 1 := by norm_cast; omega
      have h21 : (2:ℝ) / (↑(N+1)+1) ≤ 1 := (div_le_one hd).mpr h2le
      linarith
    have h_j_lower : (k : ℝ) * (log 3 / log 5) - (1 / (↑(N + 1) + 1)) ≤ (j : ℝ) := by
      have := (abs_le.mp hbound).2; linarith
    have : (j : ℝ) > 0 := by linarith
    exact Int.cast_pos.mp this
  refine ⟨k.toNat, j.toNat, ?_, ?_, ?_⟩
  ...
  · have hk_cast : (k.toNat : ℝ) = (k : ℝ) := by
    ...
    have hj_cast : (j.toNat : ℝ) = (j : ℝ) := by
      exact_mod_cast Int.toNat_of_nonneg hj_pos.le
    rw [hk_cast, hj_cast]
    have h_rearrange : (k : ℝ) * log 3 - (j : ℝ) * log 5 =
        log 5 * ((k : ℝ) * (log 3 / log 5) - (j : ℝ)) := by
      field_simp [ne_of_gt hlog5_pos]
    rw [h_rearrange, abs_mul, abs_of_pos hlog5_pos]
    calc log 5 * |(k : ℝ) * (log 3 / log 5) - (j : ℝ)|
        ≤ log 5 * (1 / (↑(N + 1) + 1)) := by
...
/-!
## Concrete bounds for (3,5): setA3 and setB5 elements
-/
private lemma setA3_le_13 {n : ℕ} (hn : n ∈ setA3) (hlt : n < 27) : n ≤ 13 := by
  simp only [setA3, Set.mem_setOf_eq] at hn
  have key : ∀ m ∈ Finset.range 27, (∀ d ∈ Nat.digits 3 m, d ≤ 1) → m ≤ 13 := by
    native_decide
  exact key n (Finset.mem_range.mpr hlt) hn

private lemma setB5_le_31 {n : ℕ} (hn : n ∈ setB5) (hlt : n < 125) : n ≤ 31 := by
  simp only [setB5, Set.mem_setOf_eq] at hn
...
/-!
## Gap in (3,5) sumset
The concrete gap {45} ⊆ ℕ \ (A_3 + B_5).
Proof: setA3∩[0,27)⊆[0,14) and setB5∩[0,125)⊆[0,32),
so any (a,b) with a+b=45 has a≤13 and b≤31, giving a+b≤44<45.
-/
lemma gap_exists_3_5 : ∃ n : ℕ, n ∉ setA3B5 := by
  use 45
  simp only [setA3B5, Set.mem_setOf_eq]
  rintro ⟨a, ha_A, b, hb_B, hab⟩
  have ha_lt : a < 27 := by omega
  have hb_lt : b < 125 := by omega
  have ha_bound : a ≤ 13 := setA3_le_13 ha_A ha_lt
  have hb_bound : b ≤ 31 := setB5_le_31 hb_B hb_lt
  omega

/-!
## Main theorem for (3,5)
-/
theorem erdos_conjecture_3_5 : ∃ n : ℕ, n ∉ setA3B5 :=
  gap_exists_3_5
