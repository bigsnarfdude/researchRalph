...
/-!
## Main Theorem: Erdős #125 for bases (3,4)
-/
...
-- Theorem: The sumset A₃ + B₅ also has a gap (where A₃ uses base 3, B₅ uses base 5)
-- Strategy: Use exact same proof structure with different bounds computed via native_decide

def setA₃ : Set ℕ := {n | ∀ d ∈ Nat.digits 3 n, d ≤ 1}
def setB₅ : Set ℕ := {n | ∀ d ∈ Nat.digits 5 n, d ≤ 1}
def setAB₃₅ : Set ℕ := {n | ∃ a ∈ setA₃, ∃ b ∈ setB₅, a + b = n}

-- For base 3: max in [0, 27) is 13 (since 3^3 = 27)
-- For base 5: max in [0, 25) is 6  (since 5^2 = 25)
-- So max sum is 13 + 6 = 19; thus 20 ∉ A₃ + B₅

private lemma setA₃_le_13 {n : ℕ} (hn : n ∈ setA₃) (hlt : n < 27) : n ≤ 13 := by
  simp only [setA₃, Set.mem_setOf_eq] at hn
...
lemma gap_exists_3_5 : ∃ n : ℕ, n ∉ setAB₃₅ := by
  ...
  have ha_bound : a ≤ 13 := setA₃_le_13 ha_A ha_lt
...
---
---
-- PHASE 2: Generalization to base pair (4, 5)
-- Theorem: The sumset A₄ + B₅ also has a gap
-- For base 4: max in [0, 16) is 5  (since 4^2 = 16, and 15 in base 4 is "33", which has digit 3 > 1; max with {0,1} is 5)
-- For base 5: max in [0, 25) is 6  (since 5^2 = 25)
-- So max sum is 5 + 6 = 11; thus 12 ∉ A₄ + B₅
...
lemma gap_exists_4_5 : ∃ n : ℕ, n ∉ setAB₄₅ := by
  use 12
  simp only [setAB₄₅, Set.mem_setOf_eq]
  rintro ⟨a, ha_A, b, hb_B, hab⟩
...
theorem erdos_125_4_5 : ∃ n : ℕ, n ∉ setAB₄₅ :=
  gap_exists_4_5

---
---
-- PHASE 2: Generalization to base pair (5, 7)
-- Theorem: The sumset A₅ + B₇ also has a gap
-- For base 5: max in [0, 25) is 6  (5^2 = 25)
-- For base 7: max in [0, 49) is 8  (7^2 = 49, max with {0,1} is (1 << 0) + (1 << 1)*7 = 1 + 7 = 8)
-- So max sum is 6 + 8 = 14; thus 15 ∉ A₅ + B₇
...
lemma gap_exists_5_7 : ∃ n : ℕ, n ∉ setAB₅₇ := by
  use 15
  simp only [setAB₅₇, Set.mem_setOf_eq]
  rintro ⟨a, ha_A, b, hb_B, hab⟩
...
theorem erdos_125_5_7 : ∃ n : ℕ, n ∉ setAB₅₇ :=
  gap_exists_5_7
