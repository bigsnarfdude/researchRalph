...
-- PHASE 2: (3,5) Base Pair Instantiation
-- =========================================
...
/-!
## Sub-lemma: 3 and 5 are multiplicatively independent over ℤ
-/
...
/-!
## Lemma 1 for (3,5): Dirichlet approximation
-/
lemma exists_k_m_ratio_close_3_5 (ε : ℝ) (hε : 0 < ε) :
  ...
  have hN2_bound : (1 : ℝ) / (↑(N + 1) + 1) < ε / log 5 := by
    ...
    push_cast; linarith
  -- j > 0 because k*(log3/log5) > log3/log5 > 0
  have hj_pos : 0 < j := by
    have hk_real : (1 : ℝ) ≤ (k : ℝ) := by exact_mod_cast hk_pos
...
For setA₃₅ ∩ [0, 81): max = 40  (since (3^4 - 1) / 2 = 40)
For setB₃₅ ∩ [0, 125): max = 31  (since (5^3 - 1) / 4 = 31)
-/
...
/-!
## Lemma 2 for (3,5): Gap in sumset
Gap: {72} (since max(setA₃₅ ∩ [0,81)) + max(setB₃₅ ∩ [0,125)) = 40 + 31 = 71 < 72)
-/
lemma gap_at_aligned_scale_3_5 (k m : ℕ) (hk : 0 < k) (hm : 0 < m)
...
/-!
## Lemma 3 for (3,5): Gap exists
72 is not in setAB₃₅
-/
lemma gap_exists_3_5 : ∃ n : ℕ, n ∉ setAB₃₅ := by
  use 72
  simp only [setAB₃₅, Set.mem_setOf_eq]
  rintro ⟨a, ha_A, b, hb_B, hab⟩
  have ha_lt : a < 81 := by omega
...
-/
theorem erdos_125_3_5 : ∃ n : ℕ, n ∉ setAB₃₅ :=
  gap_exists_3_5
