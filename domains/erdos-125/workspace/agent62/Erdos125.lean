...
/-!
## Lemma 3 (Semantic): Lower density is zero

Prove that lowerDensity(A+B) = 0 using the gap structure and Dirichlet approximation.
Strategy: Show the sequence f(N) = N^{-1} * |A+B ∩ [0,N)| tends to 0 along the
subsequence of aligned scales from exists_k_m_ratio_close.
-/
lemma independent_bases_zero_density : lowerDensity setAB = 0 := by
  simp only [lowerDensity]
  -- Apply le_antisymm: show liminf ≤ 0 and 0 ≤ liminf
  apply le_antisymm
  swap
  · -- 0 ≤ liminf (nonnegative sequence)
    exact Filter.liminf_nonneg (eventually_of_forall (fun N => by positivity))
  · -- liminf ≤ 0: use frequently-at-top with gap subsequence
    apply Filter.liminf_le_of_frequently_le
    swap
    · exact eventually_of_forall (fun N => by positivity)
    · -- At each aligned scale (k, m) from Dirichlet, the density is bounded
      rw [Filter.frequently_atTop]
      intro N₀
      -- For any N₀, find k large enough that 3^k > N₀ AND exhibits a bounded density
      obtain ⟨k, m, hk_pos, hm_pos, hclose⟩ := exists_k_m_ratio_close 1 one_pos
      -- The gap {62, 63} exists, so density at scale 3^k is bounded
      use 3^k
      constructor
      · omega
      · -- Show: (3^k)^{-1} * |setAB ∩ range(3^k)| ≤ 1 (trivial bound)
        -- For a stronger argument, we'd use gap_at_aligned_scale to get proportional gaps
        -- For now, use that |setAB ∩ [0,N)| ≤ N (every element ≤ N)
        have hcard_bound : (setAB ∩ (Finset.range (3^k)).toSet).ncard ≤ 3^k := by
          have h := Finset.card_range (3^k)
          simp only [Finset.card_le_iff_le_sum] at h ⊢
          -- The cardinality of setAB ∩ range(3^k) is at most 3^k
          omega
        sorry -- Need Filter API to connect ncard bound to density comparison
