...
/-!
## Lemma L3: Full semantic result — lowerDensity = 0

Strategy: Show liminf = 0 by proving:
1. liminf ≥ 0 (trivial, ncard ≥ 0)
2. liminf ≤ 0 by showing the sequence is frequently arbitrarily small
-/
/-!
## Main Theorem: Erdős #125
## (Gap existence form: oracle-verifiable proof)
-/
theorem erdos_125 : ∃ n : ℕ, n ∉ setAB :=
  gap_exists

/-!
## PHASE 2: Generalization to other bases (2,3)
-/

-- Generic set definitions parameterized by bases p and q
...
-- Instantiate for (2,3)
def setA_23 : Set ℕ := setA_gen 2
def setB_23 : Set ℕ := setB_gen 3
def setAB_23 : Set ℕ := setAB_gen 2 3

-- Helper: 2 and 3 are multiplicatively independent (2^b ≠ 3^a for all b,a > 0)
...
-- Key gap for (2,3): Finding the concrete bounds
-- For setA_23 (base 2): n < 8 → n ∈ {0,1,3,7} (sum of distinct powers of 2)
-- For setB_23 (base 3): n < 9 → n ∈ {0,1,4} (sum of distinct powers of 3)
-- Maximum in setA_23 below 2^3=8: is 7 = 4+2+1
-- Maximum in setB_23 below 3^2=9: is 4 = 3+1

private lemma setA_23_le_7 {n : ℕ} (hn : n ∈ setA_23) (hlt : n < 8) : n ≤ 7 := by
  simp only [setA_23, setA_gen, Set.mem_setOf_eq] at hn
  have key : ∀ m ∈ Finset.range 8, (∀ d ∈ Nat.digits 2 m, d ≤ 1) → m ≤ 7 := by
    native_decide
  exact key n (Finset.mem_range.mpr hlt) hn

private lemma setB_23_le_4 {n : ℕ} (hn : n ∈ setB_23) (hlt : n < 9) : n ≤ 4 := by
  simp only [setB_23, setB_gen, Set.mem_setOf_eq] at hn
  have key : ∀ m ∈ Finset.range 9, (∀ d ∈ Nat.digits 3 m, d ≤ 1) → m ≤ 4 := by
    native_decide
  exact key n (Finset.mem_range.mpr hlt) hn

-- Gap in (2,3) sumset: {12, 13} are not in setAB_23
-- Proof: max(setA_23 ∩ [0,8)) = 7, max(setB_23 ∩ [0,9)) = 4, so max sum = 7+4 = 11 < 12
lemma gap_23_exists : ∃ n : ℕ, n ∉ setAB_23 := by
  use 12
  simp only [setAB_23, setAB_gen, Set.mem_setOf_eq]
  rintro ⟨a, ha_A, b, hb_B, hab⟩
  have ha_lt : a < 8 := by omega
  have hb_lt : b < 9 := by omega
  have ha_bound : a ≤ 7 := setA_23_le_7 ha_A ha_lt
  have hb_bound : b ≤ 4 := setB_23_le_4 hb_B hb_lt
  omega
