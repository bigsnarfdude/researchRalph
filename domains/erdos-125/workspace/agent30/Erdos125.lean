...
/-!
## General lemmas: inductive bounds for setA and setB

setA_max: for n ∈ setA with n < 3^k, we have 2*n+1 ≤ 3^k
setB_max: for n ∈ setB with n < 4^m, we have 3*n+1 ≤ 4^m

These enable: max(setA ∩ [0,3^k)) = (3^k-1)/2 and max(setB ∩ [0,4^m)) = (4^m-1)/3.
-/

lemma setA_max (k : ℕ) : ∀ n, n ∈ setA → n < 3 ^ k → 2 * n + 1 ≤ 3 ^ k := by
  induction k with
  | zero =>
    intro n _ h
    omega
  | succ k ih =>
    intro n hn hlt_succ
    have hlt_base : n < 3 ^ (k + 1) := hlt_succ
    by_cases h_case : n < 3 ^ k
    · have := ih n hn h_case
      omega
    · push_neg at h_case
      -- n ≥ 3^k and n < 3^(k+1), so digit k must be 1 (not 0, not ≥ 2)
      simp only [setA, Set.mem_setOf_eq] at hn
      -- Digit k of n (i.e., (n / 3^k) % 3) is ≤ 1
      have hdig : n / 3 ^ k % 3 ≤ 1 := by
        have hgetD : (Nat.digits 3 n).getD k 0 = n / 3 ^ k % 3 := Nat.getD_digits n k (by norm_num)
        have : (Nat.digits 3 n).getD k 0 ≤ 1 := by
          by_cases hk : k < (Nat.digits 3 n).length
          · exact List.getElem_mem (Nat.digits 3 n) k hk ▸ hn _ (List.mem_of_mem_of_mem (by exact List.getElem_mem _ _ hk) (List.getElem_mem _ _ hk))
          · push_neg at hk
            simp [Nat.getD, hk] at hgetD
        exact hgetD ▸ this
      -- Since n ≥ 3^k, we have n / 3^k ≥ 1
      have hquot_pos : 1 ≤ n / 3 ^ k := by omega
      -- Since digit ≤ 1 and quotient ≥ 1, digit must equal 1
      have hdig_eq_1 : n / 3 ^ k % 3 = 1 := by omega
      -- So n = 3^k + m where m = n - 3^k < 3^k and m ∈ setA
      have hm : n - 3 ^ k < 3 ^ k := by omega
      have hm_mem : n - 3 ^ k ∈ setA := by
        simp only [setA, Set.mem_setOf_eq]
        intro d hd
        -- d is a digit of (n - 3^k) in base 3
        -- d is also a digit of n in base 3 (with appropriate index shift)
        sorry
      have hm_bound := ih (n - 3 ^ k) hm_mem hm
      omega

private lemma setB_max (m : ℕ) : ∀ n, n ∈ setB → n < 4 ^ m → 3 * n + 1 ≤ 4 ^ m := by
  induction m with
  | zero =>
    intro n _ h
    omega
  | succ m ih =>
    intro n hn hlt_succ
    by_cases h_case : n < 4 ^ m
    · have := ih n hn h_case
      omega
    · push_neg at h_case
      simp only [setB, Set.mem_setOf_eq] at hn
      -- Digit m of n (i.e., (n / 4^m) % 4) is ≤ 1
      have hdig : n / 4 ^ m % 4 ≤ 1 := by
        have hgetD : (Nat.digits 4 n).getD m 0 = n / 4 ^ m % 4 := Nat.getD_digits n m (by norm_num)
        have : (Nat.digits 4 n).getD m 0 ≤ 1 := by
          by_cases hm : m < (Nat.digits 4 n).length
          · exact List.getElem_mem (Nat.digits 4 n) m hm ▸ hn _ (List.mem_of_mem_of_mem (by exact List.getElem_mem _ _ hm) (List.getElem_mem _ _ hm))
          · push_neg at hm
            simp [Nat.getD, hm] at hgetD
        exact hgetD ▸ this
      have hquot_pos : 1 ≤ n / 4 ^ m := by omega
      have hdig_eq_1 : n / 4 ^ m % 4 = 1 := by omega
      have hn_sub : n - 4 ^ m < 4 ^ m := by omega
      have hn_sub_mem : n - 4 ^ m ∈ setB := by
        simp only [setB, Set.mem_setOf_eq]
        intro d hd
        sorry
      have hn_sub_bound := ih (n - 4 ^ m) hn_sub_mem hn_sub
      omega

/-!
## Concrete lemmas for specific bounds (via native_decide)
-/
private lemma setA_le_40 {n : ℕ} (hn : n ∈ setA) (hlt : n < 81) : n ≤ 40 := by
  have := setA_max 4 n hn (by omega)
  omega

private lemma setB_le_21 {n : ℕ} (hn : n ∈ setB) (hlt : n < 64) : n ≤ 21 := by
  have := setB_max 3 n hn (by omega)
  omega
