...
/-!
## Key sub-lemma: cardinality of setA at scale 3^k
setA ∩ [0, 3^k) has exactly 2^k elements (each element corresponds
to a binary choice of digits in positions 0,...,k-1).
-/
private lemma setA_card_at_scale (k : ℕ) :
    (setA ∩ (Finset.range (3^k)).toSet).ncard ≤ 2^k := by
  induction k with
  | zero =>
    simp [setA, Finset.range]
    norm_num
  | succ k ih =>
    -- setA ∩ [0, 3^(k+1)) splits into:
    -- {n < 3^k : n ∈ setA} (count ≤ 2^k by IH)
    -- {3^k + n : n < 3^k, n ∈ setA} (count ≤ 2^k by IH shifted)
    sorry

/-!
## Lemma 3 (Full): Lower density equals zero
Uses Dirichlet approximation to show that aligned gaps accumulate,
driving the density to 0.
-/
lemma independent_bases_zero_density : lowerDensity setAB = 0 := by
  rw [lowerDensity]
  -- Strategy: show that liminf of the sequence is ≤ 0
  -- We use that 0 ≤ liminf by positivity, so we just need liminf ≤ 0
  apply le_antisymm _ _
  swap
  · -- 0 ≤ liminf: free since ncard ≥ 0
    exact Filter.liminf_nonneg (eventually_of_forall (fun N => by positivity))
  · -- liminf ≤ 0: show the sequence is frequently ≤ 0+ε for small ε
    -- Key insight: use the gap sequence at aligned scales 3^k
    -- At each aligned scale, a fraction of integers are missing from setAB
    -- This forces liminf to 0

    -- For now, accept that this requires the full gap accounting:
    -- from exists_k_m_ratio_close and gap_at_aligned_scale,
    -- we can show: ∀ ε > 0, ∃ N large s.t. (3^N)⁻¹ * |setAB ∩ [0,3^N)| < ε
    sorry

/-!
## Main Theorem: Erdős #125
Weak form: gap existence
-/
theorem erdos_125 : ∃ n : ℕ, n ∉ setAB :=
  gap_exists

/-!
## Main Theorem: Erdős #125 (Full form)
Strong form: lower density equals zero
-/
theorem erdos_125_full : lowerDensity setAB = 0 :=
