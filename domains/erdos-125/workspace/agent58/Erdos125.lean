...
/-!
## Lemma 3b: Full density lemma
Extend from gap existence to the full statement: lowerDensity(A+B) = 0.
This requires showing that gaps accumulate sufficiently at exponential scales.
-/
lemma independent_bases_zero_density : lowerDensity setAB = 0 := by
  -- Strategy: show the liminf equals 0 by establishing 0 as a lower bound
  -- and showing the sequence becomes arbitrarily small along a subsequence.
  --
  -- Key observation: at each scale 3^k, the gap width grows (proportional to 3^{k-1}),
  -- enabling gaps to accumulate and push density to 0.
  --
  -- Step 1: unfold lowerDensity definition
  unfold lowerDensity

  -- Step 2: use antisymmetry to show liminf ≤ 0 (we know liminf ≥ 0 since counts ≥ 0)
  apply le_antisymm
  swap
  · -- 0 ≤ liminf: trivial since the sequence is nonnegative
    apply Filter.liminf_nonneg
    exact eventually_of_forall (fun N => by positivity)

  -- Step 3: show liminf ≤ 0 by proving the sequence is frequently ≤ 0+ε for any ε>0
  apply Filter.liminf_le_of_frequently_le
  swap
  · exact eventually_of_forall (fun N => by positivity)

  -- Step 4: use the gap structure from exists_k_m_ratio_close
  -- For any ε > 0, find scales where |k·log3 - m·log4| < 1, creating aligned gaps
  -- At these scales, (setAB ∩ range(3^k))/3^k becomes small
  rw [Filter.frequently_atTop]
  intro N₀

  -- Pick k large enough that 3^k > 2*N₀
  obtain ⟨k, m, hk, hm, h_close⟩ := exists_k_m_ratio_close 1 (by norm_num)

  -- Find k' ≥ k such that 3^k' > N₀
  obtain ⟨k', hk'_ge⟩ : ∃ k' ≥ k, N₀ < 3^k' := by
    sorry -- exists k' by taking sufficiently large power of 3

  refine ⟨3^k', ?_⟩
  simp only [mul_nonneg_iff_of_pos_left, Nat.cast_pos]

  -- Show (setAB ∩ range(3^k'))/(3^k') ≤ 1  (trivial: fraction can't exceed 1)
  -- But for liminf = 0, need something stronger: sequence actually → 0 along this subsequence
  -- This requires the full gap machinery from gap_at_aligned_scale
  sorry
...
/-!
## Main Theorem: Erdős #125 — Full density statement
This is the semantically complete version of Erdős #125.
-/
theorem erdos_125_density : lowerDensity (setAB) = 0 :=
