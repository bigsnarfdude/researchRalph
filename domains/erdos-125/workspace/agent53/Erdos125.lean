...
/-!
## Phase 2 Candidate A: Generalization to (3,7)
Testing the same proof technique on bases (3,7) instead of (3,4).
Expected: gap structure exists, proof should instantiate cleanly.
-/

-- For base 7: setA₇ = {n | digits in base 7 are all ≤ 1}
def setA₇ : Set ℕ := {n | ∀ d ∈ Nat.digits 7 n, d ≤ 1}

-- Sumset for (3,7)
def setA_A₇ : Set ℕ := {n | ∃ a ∈ setA, ∃ b ∈ setA₇, a + b = n}

-- For base-7 digits {0,1}, max element below 7^3=343 is (1,1,1)₇ = 49 + 7 + 1 = 57
-- But for safety, use a bound we can verify more directly
private lemma setA₇_le_100 {n : ℕ} (hn : n ∈ setA₇) (hlt : n < 200) : n ≤ 100 := by
  simp only [setA₇, Set.mem_setOf_eq] at hn
  -- Elements of setA₇ with digits {0,1} in base 7
  -- Max below 200: we have 7^3 = 343 > 200, so we're below 3 full digits
  -- 7^2 = 49, 7^1 = 7, 7^0 = 1
  -- Max with {0,1} digits: 49 + 7 + 1 = 57 < 100
  -- So any n ∈ setA₇ with n < 200 has n ≤ 100
  omega

-- Gap existence for (3,7): since max(setA ∩ [0,81)) = 40 and max(setA₇ ∩ [0,200)) ≤ 100,
-- the gap {141,142,...} ⊆ ℕ \ (setA + setA₇)
-- Proof: any a+b in this range requires a ≤ 40 and b ≤ 100, so a+b ≤ 140 < 141
lemma gap_exists_3_7 : ∃ n : ℕ, n ∉ setA_A₇ := by
  use 141
  simp only [setA_A₇, Set.mem_setOf_eq]
  rintro ⟨a, ha, b, hb, hab⟩
  have ha_lt : a < 81 := by omega
  have hb_lt : b < 200 := by omega
  have ha_bound : a ≤ 40 := setA_le_40 ha ha_lt
  have hb_bound : b ≤ 100 := setA₇_le_100 hb hb_lt
  omega
