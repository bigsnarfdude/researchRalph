...
--- PHASE 2: Generalization to (3,5) base pair
...
--- PHASE 2: Generalization to (3,7) base pair
...
private lemma setB₃₇_le_127 {n : ℕ} (hn : n ∈ setB₃₇) (hlt : n < 343) : n ≤ 127 := by
  simp only [setB₃₇, Set.mem_setOf_eq] at hn
  have key : ∀ m ∈ Finset.range 343, (∀ d ∈ Nat.digits 7 m, d ≤ 1) → m ≤ 127 := by
    native_decide
  exact key n (Finset.mem_range.mpr hlt) hn
...
lemma gap_exists_37 : ∃ n : ℕ, n ∉ setAB₃₇ := by
  ...
  have ha_bound : a ≤ 40 := setA₃₇_le_40 ha_A ha_lt
  have hb_bound : b ≤ 127 := setB₃₇_le_127 hb_B hb_lt
  omega
