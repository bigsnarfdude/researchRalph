...
-- Concrete test case: base pair (2,3)
...
/-!
## Sub-lemma: For coprime p,q > 1, p^b ≠ q^a for positive b,a
Generalizes nat_pow_ne from (3,4) to any coprime bases.
-/
private lemma nat_pow_ne_coprime (p q b a : ℕ) (hp : 1 < p) (hq : 1 < q)
    (hcop : Nat.Coprime p q) (hb : 0 < b) (ha : 0 < a) :
    p ^ b ≠ q ^ a := by
  intro h_eq
  have hcop_qa : Nat.Coprime p (q ^ a) := hcop.pow_right _
  have hp_dvd_qa : p ∣ q ^ a := h_eq ▸ dvd_pow_self p hb.ne'
  have hp_dvd_1 : p ∣ 1 := hcop_qa ▸ Nat.dvd_gcd (dvd_refl p) hp_dvd_qa
  have : p = 1 := Nat.eq_one_of_dvd_one hp_dvd_1
  exact absurd this (Nat.not_succ_le_zero (p - 2) (Nat.le_of_lt hp))

/-!
## Lemma 0 (parametric): irrationality of log p / log q for coprime p,q > 1
-/
private lemma irrational_log_ratio (p q : ℕ) (hp : 1 < p) (hq : 1 < q)
    (hcop : Nat.Coprime p q) :
    Irrational (log p / log q) := by
  have hlogp_pos : (0 : ℝ) < log p := Real.log_pos (Nat.one_lt_cast.mp hp)
  have hlogq_pos : (0 : ℝ) < log q := Real.log_pos (Nat.one_lt_cast.mp hq)
  rw [irrational_iff_ne_rational]
  intro a b hb heq
  have hb_real : (b : ℝ) ≠ 0 := Int.cast_ne_zero.mpr hb
  have h_mul : (b : ℝ) * log p = (a : ℝ) * log q := by
    have := div_eq_div_iff (ne_of_gt hlogq_pos) hb_real |>.mp heq
    linarith
  have ha_ne : a ≠ 0 := by
    intro ha
  ...
  have h_ne := nat_pow_ne_coprime p q b.natAbs a.natAbs hp hq hcop hb' ha'
  exact h_ne h_nat
...
/-!
## Lemma 1 (concrete for 2,3): Dirichlet approximation
-/
lemma exists_k_m_ratio_close₂₃ (ε : ℝ) (hε : 0 < ε) :
    ∃ k m : ℕ, 0 < k ∧ 0 < m ∧ |↑k * log 2 - ↑m * log 3| < ε := by
  have hlog2_pos : (0 : ℝ) < log 2 := Real.log_pos (by norm_num)
  have hlog3_pos : (0 : ℝ) < log 3 := Real.log_pos (by norm_num)
  -- log 2 / log 3 is irrational (follows from multiplicative independence of 2,3)
  have hirr : Irrational (log 2 / log 3) := irrational_log_ratio 2 3 (by norm_num) (by norm_num) (by decide)
  -- Apply Dirichlet approximation to log 2 / log 3
  ...
  have hj_pos : 0 < j := by
    ...
    -- log 2 / log 3 ≈ 0.631 > 1/2
    have hlogp_logq_half : (1:ℝ)/2 < log 2 / log 3 := by
      rw [lt_div_iff₀ hlog3_pos]
      have h1 : log 3 < log (2^2 : ℝ) := Real.log_lt_log (by norm_num) (by norm_num)
      have h2 : log (4:ℝ) = 2 * log 2 := by
        have : (4:ℝ) = 2 ^ 2 := by norm_num
        rw [this, Real.log_pow]; norm_cast
      linarith
    have hkξ_gt_half : (1:ℝ)/2 < (k:ℝ) * (log 2 / log 3) := by
      nlinarith [mul_nonneg (show (0:ℝ) ≤ (k:ℝ) - 1 by linarith) (le_of_lt hξ_pos)]
    have h_half : (1:ℝ) / (↑(N+1) + 1) ≤ 1/2 := by
      have hd : (0:ℝ) < ↑(N+1) + 1 := by positivity
      have h2le : (2:ℝ) ≤ ↑(N+1) + 1 := by norm_cast; omega
...
/-!
## Key sub-lemmas for (2,3) case: concrete bounds

For setA₂₃ (base 2): max element below 2^6=64 is 63 (= 111111₂).
For setB₂₃ (base 3): max element below 3^4=81 is 80 (= 2222₃ clamped to 1s = 1111₃ = 40).

Actually: setA₂₃ ∩ [0,64) ⊆ [0,64), setB₂₃ ∩ [0,81) ⊆ [0,81).
Need concrete bounds from enumeration.
-/
private lemma setA₂₃_le_63 {n : ℕ} (hn : n ∈ setA₂₃) (hlt : n < 64) : n ≤ 63 := by
  simp only [setA₂₃, Set.mem_setOf_eq] at hn
  have key : ∀ m ∈ Finset.range 64, (∀ d ∈ Nat.digits 2 m, d ≤ 1) → m ≤ 63 := by
    native_decide
  exact key n (Finset.mem_range.mpr hlt) hn

private lemma setB₂₃_le_40 {n : ℕ} (hn : n ∈ setB₂₃) (hlt : n < 81) : n ≤ 40 := by
  simp only [setB₂₃, Set.mem_setOf_eq] at hn
  have key : ∀ m ∈ Finset.range 81, (∀ d ∈ Nat.digits 3 m, d ≤ 1) → m ≤ 40 := by
    native_decide
  exact key n (Finset.mem_range.mpr hlt) hn
...
/-!
## Lemma 2 (for base pair 2,3): Gap in sumset
For (2,3), the gap is found at a different scale due to different base bounds.
The concrete gap: 104 is not in setAB₂₃.
Proof: setA₂₃ ∩ [0,64) ⊆ [0,64), setB₂₃ ∩ [0,81) ⊆ [0,41),
so any (a,b) with a+b=104 has a ≤ 63 and b ≤ 40, giving a+b ≤ 103 < 104.
-/
lemma gap_at_aligned_scale₂₃ (k m : ℕ) (hk : 0 < k) (hm : 0 < m)
    (h_close : |↑k * log 2 - ↑m * log 3| < 1) :
    ∃ start width : ℕ, 0 < width ∧
    ∀ n ∈ Ico start (start + width), n ∉ setAB₂₃ := by
  -- Exhibit the concrete gap {104, 105} for (2,3)
  refine ⟨104, 2, by norm_num, fun n hn hn_ab => ?_⟩
  simp only [Finset.mem_Ico] at hn
  obtain ⟨hn_lo, hn_hi⟩ := hn
  simp only [setAB₂₃, Set.mem_setOf_eq] at hn_ab
  obtain ⟨a, ha_A, b, hb_B, hab⟩ := hn_ab
  ...
  -- setA₂₃_le_63: a ∈ setA₂₃, a < 64 → a ≤ 63
  have ha_bound : a ≤ 63 := setA₂₃_le_63 ha_A ha_lt
  -- b = n - a, and n ≤ 105, a ≥ 104, so b ≤ 1. But also a + b = n so b = n - a.
  -- Actually: a + b = n, n ≥ 104, a ≤ 63, so b ≥ 41. But need b < 81.
  have hb_lt : b < 81 := by omega
  -- setB₂₃_le_40: b ∈ setB₂₃, b < 81 → b ≤ 40
  have hb_bound : b ≤ 40 := setB₂₃_le_40 hb_B hb_lt
  -- a ≤ 63 and b ≤ 40 → a+b ≤ 103, but a+b = n ≥ 104. Contradiction.
  omega

/-!
## Lemma 3 (for base pair 2,3): A gap exists in the sumset
104 is not in setAB₂₃.
-/
lemma gap_exists₂₃ : ∃ n : ℕ, n ∉ setAB₂₃ := by
  use 104
...
/-!
## Main Theorem: Erdős #125 (original 3,4 case)
-/
theorem erdos_125 : ∃ n : ℕ, n ∉ setAB :=
  gap_exists

/-!
## Phase 2 Theorem: Generalization to (2,3)
-/
theorem erdos_125_generalized₂₃ : ∃ n : ℕ, n ∉ setAB₂₃ :=
  gap_exists₂₃
