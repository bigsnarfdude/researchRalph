...
/-!
## PHASE 2: GENERALIZATION TO BASE PAIR (3,5)
-/
...
def setAC : Set ℕ := {n | ∃ a ∈ setA', ∃ c ∈ setC, a + c = n}

private lemma nat_pow_ne_35 (b a : ℕ) (hb : 0 < b) (ha : 0 < a) :
    (3 : ℕ) ^ b ≠ (5 : ℕ) ^ a := by
  intro h_eq
...
lemma exists_k_n_ratio_close_35 (ε : ℝ) (hε : 0 < ε) :
    ∃ k n : ℕ, 0 < k ∧ 0 < n ∧ |↑k * log 3 - ↑n * log 5| < ε := by
  have hlog3_pos : (0 : ℝ) < log 3 := Real.log_pos (by norm_num)
...
private lemma setA'_le_121 {n : ℕ} (hn : n ∈ setA') (hlt : n < 243) : n ≤ 121 := by
  simp only [setA', Set.mem_setOf_eq] at hn
  have key : ∀ m ∈ Finset.range 243, (∀ d ∈ Nat.digits 3 m, d ≤ 1) → m ≤ 121 := by
    native_decide
  exact key n (Finset.mem_range.mpr hlt) hn

private lemma setC_le_624 {n : ℕ} (hn : n ∈ setC) (hlt : n < 1024) : n ≤ 624 := by
  simp only [setC, Set.mem_setOf_eq] at hn
  have key : ∀ m ∈ Finset.range 1024, (∀ d ∈ Nat.digits 5 m, d ≤ 1) → m ≤ 624 := by
    native_decide
  exact key n (Finset.mem_range.mpr hlt) hn

lemma gap_exists_35 : ∃ n : ℕ, n ∉ setAC := by
  use 750
  simp only [setAC, Set.mem_setOf_eq]
  rintro ⟨a, ha_A, c, hc_C, hac⟩
  have ha_lt : a < 243 := by omega
  have hc_lt : c < 1024 := by omega
  have ha_bound : a ≤ 121 := setA'_le_121 ha_A ha_lt
  have hc_bound : c ≤ 624 := setC_le_624 hc_C hc_lt
  omega

theorem erdos_125_generalized : ∃ n : ℕ, n ∉ setAC :=
  gap_exists_35
