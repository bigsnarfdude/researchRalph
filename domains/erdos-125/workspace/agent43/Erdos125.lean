...
/-!
## PHASE 2: Instantiation for (3,7)
Parameterizing the proof to the coprime pair (3,7).
-/

-- A₃: naturals with digits {0,1} in base 3
def setA₃ : Set ℕ := {n | ∀ d ∈ Nat.digits 3 n, d ≤ 1}

-- B₇: naturals with digits {0,1} in base 7
...
/-!
## Helper lemma: 3 and 7 are multiplicatively independent
-/
private lemma nat_pow_ne_3_7 (b a : ℕ) (hb : 0 < b) (ha : 0 < a) :
    (3 : ℕ) ^ b ≠ (7 : ℕ) ^ a := by
  intro h_eq
  have hcop : Nat.Coprime 3 (7 ^ a) := (by decide : Nat.Coprime 3 7).pow_right _
  have h3_dvd_7a : (3 : ℕ) ∣ 7 ^ a := h_eq ▸ dvd_pow_self 3 hb.ne'
  have h3_dvd_1 : (3 : ℕ) ∣ 1 := hcop ▸ Nat.dvd_gcd (dvd_refl 3) h3_dvd_7a
  exact absurd h3_dvd_1 (by decide)

/-!
## Lemma 1 for (3,7): Dirichlet approximation
-/
lemma exists_k_m_ratio_close_3_7 (ε : ℝ) (hε : 0 < ε) :
    ∃ k m : ℕ, 0 < k ∧ 0 < m ∧ |↑k * log 3 - ↑m * log 7| < ε := by
  have hlog3_pos : (0 : ℝ) < log 3 := Real.log_pos (by norm_num)
  have hlog7_pos : (0 : ℝ) < log 7 := Real.log_pos (by norm_num)
  -- log 3 / log 7 is irrational
  have hirr : Irrational (log 3 / log 7) := by
    rw [irrational_iff_ne_rational]
    intro a b hb heq
    have hb_real : (b : ℝ) ≠ 0 := Int.cast_ne_zero.mpr hb
    have h_mul : (b : ℝ) * log 3 = (a : ℝ) * log 7 := by
    ...
    have hb' : 0 < b.natAbs := Int.natAbs_pos.mpr hb
    have ha' : 0 < a.natAbs := Int.natAbs_pos.mpr ha_ne
    have h_natabs : (b.natAbs : ℝ) * log 3 = (a.natAbs : ℝ) * log 7 := by
      rw [Nat.cast_natAbs, Nat.cast_natAbs, Int.cast_abs, Int.cast_abs]
      obtain hb_nn | hb_neg := le_or_gt 0 (b : ℝ)
      · rw [abs_of_nonneg hb_nn]
        have ha_nn : 0 ≤ (a : ℝ) := by nlinarith
      ...
      · rw [abs_of_neg hb_neg]
        have ha_neg : (a : ℝ) < 0 := by nlinarith
        rw [abs_of_neg ha_neg]; linarith
    have h_rpow : (3 : ℝ) ^ b.natAbs = (7 : ℝ) ^ a.natAbs := by
      apply Real.log_injOn_pos (Set.mem_Ioi.mpr (by positivity))
                               (Set.mem_Ioi.mpr (by positivity))
      rw [Real.log_pow, Real.log_pow]
      exact_mod_cast h_natabs
    have h_nat : (3 : ℕ) ^ b.natAbs = (7 : ℕ) ^ a.natAbs := by exact_mod_cast h_rpow
    have hcop : Nat.Coprime 3 (7 ^ a.natAbs) := (by decide : Nat.Coprime 3 7).pow_right _
    have h3_dvd : (3 : ℕ) ∣ 7 ^ a.natAbs := h_nat ▸ dvd_pow_self 3 hb'.ne'
    exact absurd (hcop ▸ Nat.dvd_gcd (dvd_refl 3) h3_dvd) (by decide)
  -- Dirichlet approximation
  obtain ⟨N, hN⟩ := exists_nat_gt (log 7 / ε)
  have hN_pos : 0 < N + 1 := Nat.succ_pos _
  obtain ⟨j, k, hk_pos, _, hbound⟩ :=
    Real.exists_int_int_abs_mul_sub_le (log 3 / log 7) hN_pos
  -- 1/(N+2) < ε/log7
  have hN2_bound : (1 : ℝ) / (↑(N + 1) + 1) < ε / log 7 := by
    ...
    push_cast; linarith
  -- j > 0 because k*(log3/log7) > 1/3 > 1/(N+2) ≥ 0
  have hj_pos : 0 < j := by
    have hk_real : (1 : ℝ) ≤ (k : ℝ) := by exact_mod_cast hk_pos
    have hξ_pos : 0 < log 3 / log 7 := div_pos hlog3_pos hlog7_pos
    have hξ_gt_third : (1:ℝ)/3 < log 3 / log 7 := by
      rw [lt_div_iff₀ hlog7_pos]
      have h1 : log 7 < log 27 := Real.log_lt_log (by norm_num) (by norm_num)
      have h2 : log (27:ℝ) = 3 * log 3 := by
        have : (27:ℝ) = 3 ^ 3 := by norm_num
        rw [this, Real.log_pow]; norm_cast
      linarith
    have hkξ_gt_third : (1:ℝ)/3 < (k:ℝ) * (log 3 / log 7) := by
      nlinarith [mul_nonneg (show (0:ℝ) ≤ (k:ℝ) - 1 by linarith) (le_of_lt hξ_pos)]
    have h_third : (1:ℝ) / (↑(N+1) + 1) ≤ 1/3 := by
      have hd : (0:ℝ) < ↑(N+1) + 1 := by positivity
      have h3le : (3:ℝ) ≤ ↑(N+1) + 1 := by norm_cast; omega
      have h31 : (3:ℝ) / (↑(N+1)+1) ≤ 1 := (div_le_one hd).mpr h3le
      linarith [show (1:ℝ) / (↑(N+1)+1) = (3:ℝ) / (↑(N+1)+1) / 3 from by ring]
    have h_j_lower : (k : ℝ) * (log 3 / log 7) - (1 / (↑(N + 1) + 1)) ≤ (j : ℝ) := by
      have := (abs_le.mp hbound).2; linarith
    have : (j : ℝ) > 0 := by linarith
    exact Int.cast_pos.mp this
  refine ⟨k.toNat, j.toNat, ?_, ?_, ?_⟩
  ...
  · have hk_cast : (k.toNat : ℝ) = (k : ℝ) := by
    ...
    have hj_cast : (j.toNat : ℝ) = (j : ℝ) := by
      exact_mod_cast Int.toNat_of_nonneg hj_pos.le
    rw [hk_cast, hj_cast]
    have h_rearrange : (k : ℝ) * log 3 - (j : ℝ) * log 7 =
        log 7 * ((k : ℝ) * (log 3 / log 7) - (j : ℝ)) := by
      field_simp [ne_of_gt hlog7_pos]
    rw [h_rearrange, abs_mul, abs_of_pos hlog7_pos]
    calc log 7 * |(k : ℝ) * (log 3 / log 7) - (j : ℝ)|
        ≤ log 7 * (1 / (↑(N + 1) + 1)) := by
...
/-!
## Concrete bounds for (3,7): Maximum elements in finite ranges
Using 3^4=81 for setA and 7^2=49 for setB.
-/
private lemma setA₃_le_40_at_81 {n : ℕ} (hn : n ∈ setA₃) (hlt : n < 81) : n ≤ 40 := by
  simp only [setA₃, Set.mem_setOf_eq] at hn
  have key : ∀ m ∈ Finset.range 81, (∀ d ∈ Nat.digits 3 m, d ≤ 1) → m ≤ 40 := by
    native_decide
  exact key n (Finset.mem_range.mpr hlt) hn

private lemma setB₇_le_8_at_49 {n : ℕ} (hn : n ∈ setB₇) (hlt : n < 49) : n ≤ 8 := by
  simp only [setB₇, Set.mem_setOf_eq] at hn
...
/-!
## Lemma 2 for (3,7): Gap in sumset
The concrete gap {49} ⊆ ℕ \ setAB₃₇.
Proof: setA₃∩[0,81)⊆[0,41) and setB₇∩[0,49)⊆[0,9),
so any (a,b) with a+b=49 has a≤40 and b≤8, giving a+b≤48<49.
-/
lemma gap_at_aligned_scale_3_7 (k m : ℕ) (hk : 0 < k) (hm : 0 < m)
    (h_close : |↑k * log 3 - ↑m * log 7| < 1) :
    ∃ start width : ℕ, 0 < width ∧
    ∀ n ∈ Ico start (start + width), n ∉ setAB₃₇ := by
  -- Exhibit the concrete gap {49}
  refine ⟨49, 1, by norm_num, fun n hn hn_ab => ?_⟩
  simp only [Finset.mem_Ico] at hn
  obtain ⟨hn_lo, hn_hi⟩ := hn
  simp only [setAB₃₇, Set.mem_setOf_eq] at hn_ab
  obtain ⟨a, ha_A, b, hb_B, hab⟩ := hn_ab
  -- a + b = 49, so a < 49 < 81 and b < 49
  have ha_lt : a < 81 := by omega
  have hb_lt : b < 49 := by omega
  have ha_bound : a ≤ 40 := setA₃_le_40_at_81 ha_A ha_lt
  have hb_bound : b ≤ 8 := setB₇_le_8_at_49 hb_B hb_lt
  omega

/-!
## Lemma 3 for (3,7): A gap exists
49 is not in setAB₃₇.
-/
lemma gap_exists_3_7 : ∃ n : ℕ, n ∉ setAB₃₇ := by
  use 49
  simp only [setAB₃₇, Set.mem_setOf_eq]
  rintro ⟨a, ha_A, b, hb_B, hab⟩
  have ha_lt : a < 81 := by omega
...
/-!
## Main Theorem for (3,7): Erdős #125 for bases (3,7)
-/
theorem erdos_125_3_7 : ∃ n : ℕ, n ∉ setAB₃₇ :=
  gap_exists_3_7
