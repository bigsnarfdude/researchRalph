...
/-!
## PHASE 2: Generalization to base pair (3,7)

Instantiate the gap-existence proof for bases 3 and 7 (multiplicatively independent).
This demonstrates the technique applies beyond (3,4).
-/
...
/-! Bounds for (3,7): max(setA₃₇ ∩ [0, 3^5)) = 121, max(setB₃₇ ∩ [0, 7^3)) = 57 -/
...
private lemma setB₃₇_le_57 {n : ℕ} (hn : n ∈ setB₃₇) (hlt : n < 343) : n ≤ 57 := by
  simp only [setB₃₇, Set.mem_setOf_eq] at hn
...
lemma gap_exists₃₇ : ∃ n : ℕ, n ∉ setAB₃₇ := by
  ...
  simp only [setAB₃₇, Set.mem_setOf_eq]
  rintro ⟨a, ha_A, b, hb_B, hab⟩
  -- a ∈ setA₃₇, so a ≤ 121 (for n=179, a < 243)
  have ha_lt : a < 243 := by omega
  have hb_lt : b < 343 := by omega
  have ha_bound : a ≤ 121 := setA₃₇_le_121 ha_A ha_lt
  have hb_bound : b ≤ 57 := setB₃₇_le_57 hb_B hb_lt
  -- a ≤ 121 and b ≤ 57 → a+b ≤ 178 < 179
  omega
...
/-!
## Phase 2 continued: Base pair (5,7)

Further validation of instantiation approach.
-/
...
private lemma setA₅₇_le_312 {n : ℕ} (hn : n ∈ setA₅₇) (hlt : n < 625) : n ≤ 312 := by
  simp only [setA₅₇, Set.mem_setOf_eq] at hn
  have key : ∀ m ∈ Finset.range 625, (∀ d ∈ Nat.digits 5 m, d ≤ 1) → m ≤ 312 := by
    native_decide
  exact key n (Finset.mem_range.mpr hlt) hn
...
lemma gap_exists₅₇ : ∃ n : ℕ, n ∉ setAB₅₇ := by
  ...
  simp only [setAB₅₇, Set.mem_setOf_eq]
  rintro ⟨a, ha_A, b, hb_B, hab⟩
  have ha_lt : a < 625 := by omega
  have hb_lt : b < 343 := by omega
  have ha_bound : a ≤ 312 := setA₅₇_le_312 ha_A ha_lt
  have hb_bound : b ≤ 57 := setB₅₇_le_57 hb_B hb_lt
  omega
