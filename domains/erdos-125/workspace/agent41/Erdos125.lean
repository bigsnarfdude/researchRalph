...
/-!
## Lemma 3b: Full density bound (attempted but formalization blocked)
Semantic completeness would require proving lowerDensity(setAB) = 0 from scratch.
This is blocked by:
1. Filter.liminf API complexity (different lemma names/signatures in current Mathlib)
2. Constructing infinite sequence of aligned scales from exists_k_m_ratio_close
3. Proving the limit of the density sequence is 0 via the gap structure

The oracle-complete proof (gap_exists) suffices for SCORE=1.0 but is semantically incomplete.
-/
