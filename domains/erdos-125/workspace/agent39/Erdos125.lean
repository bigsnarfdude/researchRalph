...
/-!
## PHASE 2: GENERALIZATION TO COPRIME BASES (3,5)

Testing whether the (3,4) proof technique generalizes to other multiplicatively independent base pairs.
Note: base 2 doesn't work because all binary digits are automatically in {0,1}.
Using (3,5) instead: both bases have digits > 1, so the digit constraint is properly restrictive.
-/

namespace Phase2Bases35

def setA_35 : Set ℕ := {n | ∀ d ∈ Nat.digits 3 n, d ≤ 1}

def setB_35 : Set ℕ := {n | ∀ d ∈ Nat.digits 5 n, d ≤ 1}

def setAB_35 : Set ℕ := {n | ∃ a ∈ setA_35, ∃ b ∈ setB_35, a + b = n}

-- 3 and 5 are coprime, so 3^a ≠ 5^b for positive a, b
...
-- Dirichlet approximation for log 3 / log 5
...
-- Concrete bounds for (3,5) case: setA_35 ∩ [0,81) ⊆ [0,41), setB_35 ∩ [0,125) ⊆ [0,63)
private lemma setA_35_le_40 {n : ℕ} (hn : n ∈ setA_35) (hlt : n < 81) : n ≤ 40 := by
  simp only [setA_35, Set.mem_setOf_eq] at hn
  have key : ∀ m ∈ Finset.range 81, (∀ d ∈ Nat.digits 3 m, d ≤ 1) → m ≤ 40 := by
    native_decide
  exact key n (Finset.mem_range.mpr hlt) hn

private lemma setB_35_le_31 {n : ℕ} (hn : n ∈ setB_35) (hlt : n < 125) : n ≤ 31 := by
  simp only [setB_35, Set.mem_setOf_eq] at hn
  have key : ∀ m ∈ Finset.range 125, (∀ d ∈ Nat.digits 5 m, d ≤ 1) → m ≤ 31 := by
    native_decide
  exact key n (Finset.mem_range.mpr hlt) hn

-- Gap existence for (3,5)
lemma gap_exists_35 : ∃ n : ℕ, n ∉ setAB_35 := by
  use 72
  simp only [setAB_35, Set.mem_setOf_eq]
  rintro ⟨a, ha_A, b, hb_B, hab⟩
  have ha_lt : a < 81 := by omega
  have hb_lt : b < 125 := by omega
  have ha_bound : a ≤ 40 := setA_35_le_40 ha_A ha_lt
  have hb_bound : b ≤ 31 := setB_35_le_31 hb_B hb_lt
  omega

theorem erdos_125_35 : ∃ n : ℕ, n ∉ setAB_35 :=
  gap_exists_35
