...
---
/- PHASE 2: CANDIDATE A — Instantiation for (3,7) pair -/

/-!
## (3,7) Instantiation: Bases 3 and 7

Following the successful pattern from (3,4), (3,5), (4,5), (5,7):
instantiate the proof for multiplicatively independent base pair (3,7).

setA₃: natural numbers with only digits {0,1} in base 3
setB₇: natural numbers with only digits {0,1} in base 7

Goal: prove ∃ n ∉ setA₃ + setB₇
-/
...
/-!
## Concrete bounds for (3,7)

Range selection (must pick k, m such that aligned scale exists):
- For base 3: 3^4 = 81, and max of setA₃ below 81 is (81-1)/2 = 40
- For base 7: 7^2 = 49, and max of setB₇ below 49 is (49-1)/6 = 8

These bounds are verified via native_decide (decidable digit membership).
-/
...
Concrete gap: {49, 50}.
Logic: max(setA₃₇ ∩ [0,81)) = 40, max(setB₃₇ ∩ [0,49)) = 8.
So max sum = 40 + 8 = 48 < 49. Therefore 49 ∉ setAB₃₇.
-/

lemma gap_exists_37 : ∃ n : ℕ, n ∉ setAB₃₇ := by
  use 49
  simp only [setAB₃₇, Set.mem_setOf_eq]
  rintro ⟨a, ha_A, b, hb_B, hab⟩
  -- From a + b = 49, we get constraints
  have ha_lt : a < 81 := by omega
  have hb_lt : b < 49 := by omega
  have ha_bound : a ≤ 40 := setA₃₇_le_40 ha_A ha_lt
  have hb_bound : b ≤ 8 := setB₃₇_le_8 hb_B hb_lt
  -- a ≤ 40 and b ≤ 8 means a + b ≤ 48, contradicting a + b = 49
  linarith

/-!
## Main theorem for (3,7)
-/
theorem erdos_125_37 : ∃ n : ℕ, n ∉ setAB₃₇ :=
  gap_exists_37
