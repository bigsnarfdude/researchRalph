...
/-!
## Lemma 3 Upgrade: General gap lemma with scale-dependent bounds
-/

-- Maximum element of setA below 3^k is at most (3^k - 1) / 2
-- For k ≥ 1, if all digits of n in base 3 are ≤ 1, and n < 3^k,
-- then n ≤ sum of powers 3^0 + 3^1 + ... + 3^(k-1) = (3^k - 1) / 2
private lemma setA_max (k : ℕ) (hk : 0 < k) :
    ∀ n ∈ setA, n < 3^k → n ≤ (3^k - 1) / 2 := by
  intro n hn hlt
  simp only [setA, Set.mem_setOf_eq] at hn
  ...
  | 0 => omega
  | 1 =>
    -- For k=1: n < 3, digits ≤ 1, so n ∈ {0, 1, 2}. But digit constraint → n ∈ {0, 1}
    simp only at hn
    have : n ≤ 1 := by
      by_contra h; push_neg at h
      have h2 : 2 ≤ n := by omega
      have h3 : n < 3 := hlt
      have : 1 < n / 3 ∨ 1 ≤ n % 3 := by omega
      cases this with
      | inl _ => omega
      | inr h_rem =>
        have hd : 1 ≤ n % 3 := h_rem
        have : n % 3 ∈ Nat.digits 3 n := by
          rw [Nat.mem_digits (by norm_num) (Nat.pos_iff_ne_zero.mpr (by omega))]
    ...
    exact this
  | k + 2 =>
    -- General case: reduce to k+1
    sorry

-- Maximum element of setB below 4^m is at most (4^m - 1) / 3
private lemma setB_max (m : ℕ) (hm : 0 < m) :
    ∀ n ∈ setB, n < 4^m → n ≤ (4^m - 1) / 3 := by
  intro n hn hlt
  simp only [setB, Set.mem_setOf_eq] at hn
  -- Elements of setB are sums of distinct powers of 4
  -- The maximum is 1 + 4 + 4^2 + ... + 4^(m-1) = (4^m - 1) / 3
  sorry

-- General gap lemma: at aligned scales, gap size is proportional to min(3^k, 4^m)
lemma gap_at_aligned_scale_general (k m : ℕ) (hk : 0 < k) (hm : 0 < m) :
    ∃ gap_start gap_width : ℕ, 0 < gap_width ∧
    (gap_start + gap_width ≤ 2 * (3^k) ∨ gap_start + gap_width ≤ 2 * (4^m)) ∧
    ∀ n ∈ Ico gap_start (gap_start + gap_width), n ∉ setAB := by
  -- Use setA_max and setB_max to bound the sumset
  -- If max(setA ∩ [0, 3^k)) ≤ (3^k - 1)/2 and max(setB ∩ [0, 4^m)) ≤ (4^m - 1)/3,
  -- then max sum < (3^k - 1)/2 + (4^m - 1)/3
  -- Create gap at [(3^k - 1)/2 + (4^m - 1)/3 + 1, min(3^k, 4^m))
  sorry
