...
/-!
## PHASE 2: Generalization to other base pairs
-/

namespace Phase2Exploration

/-!
## Phase 2: Generalization to Other Base Pairs

The Phase 1 proof (3,4) is concrete and uses native_decide for digit bounds.
Testing generalization: can we find gaps for (2,3), (2,5) using similar logic?

Key mathematical insight: For coprime p,q > 1 with log(p)/log(q) irrational,
Dirichlet approximation guarantees aligned scales where p^k ≈ q^m.
At such scales, the gap structure from Phase 1 should repeat.

Challenge: Generalization requires proving lowerDensity = 0, not just ∃ n ∉ setAB.
The Phase 1 proof only shows ∃ 62 ∉ A+B, not the full density bound.
-/

-- Generalized set for base p
def setA_p (p : ℕ) : Set ℕ := {n | ∀ d ∈ Nat.digits p n, d ≤ 1}

-- Generalized set for base q
def setB_q (q : ℕ) : Set ℕ := {n | ∀ d ∈ Nat.digits q n, d ≤ 1}

-- Sumset for arbitrary bases
def setSumset_pq (p q : ℕ) : Set ℕ :=
  {n | ∃ a ∈ setA_p p, ∃ b ∈ setB_q q, a + b = n}

/-! ### Observation: (2,3) has an aligned scale at (2^8, 3^5) -/

-- 2^8 = 256 and 3^5 = 243 are close (ratio ≈ 1.053)
-- Max element of setA_p 2 below 256: 255 (all 1s in binary: 11111111)
-- Max element of setB_q 3 below 243: 121 (111 in base 3 = 121)
private lemma setA_p_2_bounded : ∀ n ∈ setA_p 2, n < 256 → n ≤ 255 := by
  intro n hn hlt
  simp only [setA_p, Set.mem_setOf_eq] at hn
  have key : ∀ m ∈ Finset.range 256, (∀ d ∈ Nat.digits 2 m, d ≤ 1) → m ≤ 255 := by
    native_decide
  exact key n (Finset.mem_range.mpr hlt) hn

private lemma setB_q_3_bounded : ∀ n ∈ setB_q 3, n < 243 → n ≤ 121 := by
  intro n hn hlt
  simp only [setB_q, Set.mem_setOf_eq] at hn
  have key : ∀ m ∈ Finset.range 243, (∀ d ∈ Nat.digits 3 m, d ≤ 1) → m ≤ 121 := by
    native_decide
  exact key n (Finset.mem_range.mpr hlt) hn

/-! ### Existence of a gap in setSumset(2,3) -/

lemma gap_exists_2_3 : ∃ n : ℕ, n ∉ setSumset_pq 2 3 := by
  -- Gap at 377: max(setA_p 2 below 256) + max(setB_q 3 below 243) = 255 + 121 = 376
  use 377
  simp only [setSumset_pq, Set.mem_setOf_eq]
  rintro ⟨a, ha_A, b, hb_B, hab⟩
  -- Key insight: a < 256 (otherwise the gap wouldn't apply)
  -- From a + b = 377 and a < 256, we get b = 377 - a > 121
  -- But setB_q 3 with n < 243 has max 121, so b ≥ 243
  -- This means we must have a ≥ 256 or b ≥ 243
  by_cases ha : a < 256
  · -- Case a < 256: apply the bound
    have ha_bound : a ≤ 255 := setA_p_2_bounded a ha_A ha
    have hb_val : b = 377 - a := by omega
    rw [hb_val] at hab
    -- Now 377 - a < 377 - 0 = 377, so b < 377
    -- And from a ≤ 255: 377 - 255 ≤ b, so 122 ≤ b
    -- But if b < 243, then b ≤ 121 by setB_q_3_bounded
    by_cases hb : b < 243
    · have hb_bound : b ≤ 121 := setB_q_3_bounded b hb_B hb
      omega
    · -- b ≥ 243, which is consistent. No contradiction here.
      skip
  · -- Case a ≥ 256: This is allowed for setA_p 2 (no upper bound on membership)
    -- So we can't derive a contradiction just from the bounds.
    -- This suggests 377 might actually be in the sumset for larger a values.
    skip

end Phase2Exploration
